...
## Tab 5 — Profile 🔧 (redesign built, not yet verified on device)

The profile screen has been fully redesigned and built. The dimension bar chart is replaced by the status surface + focus area card stack.

### New Profile Structure 🔧

```
Profile
  ├── Sticky header (profile label + settings icon top-right) ✅
  ├── Status area — background surface, not a card ✅ built
  │     ├── Avatar (squircle, 60×60px, 18px radius) ✅ built → openPicPicker()
  │     ├── Level eyebrow (level name + assessed note) ✅ built
  │     ├── Level animal watermark (7% opacity, right-aligned) ✅ built
  │     └── Insight sentence (event-driven priority queue) ✅ built
  │           Priority: session today → milestone → correction pattern → assessment fallback
  │
  └── Focus area card stack ✅ built
        Cards anchored to bottom. Top corners 12px rounded, bottom square.
        Order back→front: Pointe · Artistry · The Body · Movement · Technique
        Each card → bottom sheet on tap ✅ built
        Sheet sections: where you are, what this means, focus, corrections,
                        goals, connected sessions, how this fits, assessment history
        Jump tabs: only rendered where data exists ✅ built
        Pointe: dismiss button → sets hidePointe=true, toast notification ✅ built

        Card states:
          ├── Fully assessed — full card with track + stats ✅ built
          ├── Partially assessed — assessed subdims shown, nudge for rest ✅ built
          ├── Unassessed — compact (46px header), sheet has assess CTA ✅ built
          └── Pointe opt-in — dimmed, restores via assessment ✅ built
```
...
## Settings Screen 🔧 (built, not yet verified on device)

Accessible via settings icon in the profile sticky header. Bottom sheet, same pattern as session logger.

```
Settings
  ├── Profile
  │     ├── Profile picture → opens picture picker sheet 🔧
  │     │     ├── Choose from 3 default cameo illustrations 🔧
  │     │     ├── Upload photo (FileReader → data URL) 🔧
  │     │     └── 9 more defaults outstanding — see illustration brief
  │     └── Display name (prompt-based edit) 🔧
  │
  ├── My training
  │     └── Pointe toggle → hidePointe flag → hides pointe everywhere 🔧
  │
  ├── Notifications ⬜
  │     ├── Session reminders ⬜
  │     ├── Reflection prompts ⬜
  ...
  │
  ├── Display ⬜
  │     └── Dark mode ⬜ (planned, not in CSS yet)
  │
  ├── Data
  │     ├── Export training log (PDF/CSV) ⬜
  │     ├── Import data ⬜
  │     └── Reset all data 🔧 (moved here from profile bottom)
  │
...
### Data model changes — built this session ✅

```js
// appState additions — built
appState.hidePointe: Boolean         // hides pointe everywhere in app ✅
appState.profilePicture: String      // data URL or default key ✅
appState.displayName: String         // optional display name ✅

// STORAGE_KEYS additions — built
'plie:preferences'                   // { hidePointe, profilePicture, displayName } ✅
```

### Data model changes — still deferred 📋
