...
| Token | Hex | Job |
|---|---|---|
| `--gold` | `#C4900C` | Active nav state, category labels on skill cards, highlight star (active), key point dashes in Learn, input focus border |
| `--gold-soft` | `#E8D5A8` | Input border at rest |
| `--gold-wash` | `#F5EDD6` | Level badge background, goal badge background, soft button fill |
| `--coral` | `#A84030` | Correction source label, delete action, alert/error state |
| `--coral-soft` | `#F0D5CF` | Recurring note row background at reduced opacity |
| `--sage` | `#4A7038` | Completed goal indicator, ticked progress marker |
| `--brown-btn` | `#5A4030` | All primary action buttons and FAB only |
| `--ink` | `#1A1714` | Primary text, actions |
| `--ink-2` | `#2D2520` | Body text |
| `--ink-3` | `#3D3530` | Secondary body text |
| `--ink-4` | `#5C5149` | Tertiary text |
| `--ink-5` | `#9A8E87` | Metadata, captions, hint text |
| `--ink-6` | `#C8BFB8` | Disabled states, decorative dots |
...
Regular (400) and semibold (600) only. Never 700.
...
Block type label: set at creation (Correction / Observation / Note / Goal). Small-caps DM Sans 600, `--ink-5`, above the block header.
...
| Element | Decision |
|---|---|
| Block types | Correction / Observation / Note / Goal |
